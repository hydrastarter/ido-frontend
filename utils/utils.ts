...
export const toTimestamp = (d: Date): string =>
  `${d.getFullYear()}-${d.getMonth() > 9 ? d.getMonth() : `0${d.getMonth()}`}-${
    d.getUTCDay() > 9 ? d.getUTCDay() : `0${d.getUTCDay()}`
  }T${d.getHours()}:${d.getMinutes()}:${d.getSeconds()}+00:00`;
...
export const toDecimal = (num: string | number, decimals: number): number => {
  ...
  if (n.length > decimals) {
    const val = `${n.slice(0, n.length - decimals)}.${n.slice(
      n.length - decimals,
      n.length
    )}`;
